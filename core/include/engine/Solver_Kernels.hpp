...
oso_calc_gradients_async( vectorfield_view grad, const_vectorfield_view spins, const_vectorfield_view forces )
{
    const Matrix3 t = ( Matrix3() << 0, 0, 1, 0, -1, 0, 1, 0, 0 ).finished();

    return stdexec::bulk( grad.size(), [=]( std::size_t i ) { grad[i] = t * ( -spins[i].cross( forces[i] ) ); } );
}

void oso_rotate(
    Execution::Context, std::vector<std::shared_ptr<vectorfield>> & configurations,
