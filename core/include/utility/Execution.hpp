...
struct Void_Context
{
    static Void_Schedule get_scheduler() noexcept
    {
        return Void_Schedule{};
    }
};
...
public:
    ...
    explicit Compute_Resource( int = 0 ) {}
...
public:
    ...
    explicit Compute_Resource() : shape_{ .devices = 1 }
    {
...
public:
    ...
    explicit Compute_Resource( int num_threads = std::thread::hardware_concurrency() )
            : shape_{ .threads = num_threads }, resource_( num_threads )
    {
    }
...
public:
    ...
    [[nodiscard]] auto get_scheduler()
    {
        assert( res_ != nullptr );
        return res_->resource_.get_scheduler();
    }

    [[nodiscard]] Resource_Shape resource_shape() const noexcept
    {
