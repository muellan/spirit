...
TEST_CASE("exec reduce uses passed product operation correctly", "[adaptors][reduce]") {
  constexpr int N = 15;
  ...
  exec::static_thread_pool pool{};
  auto task = stdexec::transfer_just(pool.get_scheduler(), std::span{input})
            | exec::reduce(init, exec::reduction::product);

  auto [result] = stdexec::sync_wait(std::move(task)).value();

  REQUIRE(result == 3 * static_cast<int>(std::pow(2, 15)));
}

TEST_CASE("exec reduce uses passed min operation correctly", "[adaptors][reduce]") {
  constexpr int N = 2048;
  constexpr int init = 47;
  ...
  exec::static_thread_pool pool{};
  auto task = stdexec::transfer_just(pool.get_scheduler(), std::span{input})
            | exec::reduce(init, exec::reduction::min);
...
TEST_CASE("exec reduce uses passed max operation correctly", "[adaptors][reduce]") {
  constexpr int N = 2048;
  constexpr int init = 47;
  ...
  exec::static_thread_pool pool{};
  auto task = stdexec::transfer_just(pool.get_scheduler(), std::span{input})
            | exec::reduce(init, exec::reduction::max);
...
TEST_CASE("exec reduce uses a passed custom reduction operation", "[adaptors][reduce]") {
  constexpr int N = 2048;
  constexpr int init = 47;
