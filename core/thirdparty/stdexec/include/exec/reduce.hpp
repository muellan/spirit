...
namespace exec {
  ...
  namespace __reduce {
      ...
      struct __t {
        ...
        friend void tag_invoke(_Tag, __t&& __self, _Range&& __range) noexcept {
          auto result = std::reduce(
            ...
            __self.__op_->__init_,
            __self.__op_->__redop_);
    ...
    concept reduction_operation =
      std::invocable<Op, Value, Value>
      && std::convertible_to<std::invoke_result_t<Op, Value, Value>, Value>;

    namespace __reduction {
      struct sum_t {
        template <class A, class B>
        STDEXEC_DETAIL_CUDACC_HOST_DEVICE decltype(auto) operator()(A&& a, B&& b) const {
          return (A&&) a + (B&&) b;
        }
      };
      ...
      struct max_t {
        template <class A, class B>
        STDEXEC_DETAIL_CUDACC_HOST_DEVICE auto operator()(A&& a, B&& b) const {
          using std::max;
          return max(a, b);
        }
      };
    ...
    struct reduce_t {
      ...
      template <stdexec::sender _Sender, class _InitT, stdexec::__movable_value _RedOp>
        requires stdexec::__tag_invocable_with_completion_scheduler<
                   ...
                   _InitT,
                   _RedOp>
              && reduction_operation<_RedOp, _InitT>
      ...
      template <class _InitT, class _RedOp = __reduction::sum_t>
        requires reduction_operation<_RedOp, _InitT>
      stdexec::__binder_back<reduce_t, _InitT, _RedOp>
        operator()(_InitT __init, _RedOp __redop = {}) const {
        return {
          {},
          {},
  ...
  namespace reduction {
    inline constexpr __reduce::__reduction::sum_t sum{};
    inline constexpr __reduce::__reduction::product_t product{};
    inline constexpr __reduce::__reduction::min_t min{};
    inline constexpr __reduce::__reduction::max_t max{};
  }
